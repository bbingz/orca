...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    '',
    'let lastStatus = "idle";',
    'const parentSessionById = new Map();',
    'const activeChildIDsByParentID = new Map();',
    '',
    ...
    '// own session.idle / message events. Child message events never replace',
    '// the root preview, but child status must keep that specific parent busy.',
    '// Cache the actual parentID and fail closed on lookup errors so a transient',
    '// SDK failure cannot create a false root "done" transition.',
    'async function getParentSessionID(client, sessionID) {',
    '  if (!sessionID) return undefined;',
    '  if (parentSessionById.has(sessionID)) return parentSessionById.get(sessionID);',
    '  if (!client?.session?.list) return undefined;',
    '  try {',
    ...
    '    if (parentSessionById.size >= 128) {',
    '      const first = parentSessionById.keys().next().value;',
    '      if (first !== undefined) parentSessionById.delete(first);',
    '    }',
    '    parentSessionById.set(sessionID, parentID);',
    '    return parentID;',
    '  } catch {',
    '    return undefined;',
    '  }',
    '}',
    '',
    'function setChildActive(parentID, childID, active) {',
    '  let childIDs = activeChildIDsByParentID.get(parentID);',
    '  if (active) {',
    '    if (!childIDs) {',
    '      childIDs = new Set();',
    '      activeChildIDsByParentID.set(parentID, childIDs);',
    '    }',
    ...
    '  if (childIDs.size === 0) activeChildIDsByParentID.delete(parentID);',
    '}',
    '',
    'function hasActiveChildren(sessionID) {',
    '  return activeChildIDsByParentID.get(sessionID)?.size > 0;',
    '}',
    '',
    ...
    '',
    '    // Why: cache the message role BEFORE the async parent lookup.',
    '    // OpenCode fires message.updated (user) and message.part.updated (text)',
    '    // back-to-back; if we awaited the parent lookup first, the part.updated',
    ...
    '    }',
    '',
    '    // Why: user requests bypass parent lookup but do not change lastStatus, preview, or done state.',
    ...
    '    const parentSessionID = sessionID ? await getParentSessionID(client, sessionID) : null;',
    '',
    '    if (sessionID && parentSessionID === undefined) {',
    ...
    '        const statusType = getStatusType(event);',
    '        if (statusType === "busy" || statusType === "retry") setChildActive(parentSessionID, sessionID, true);',
    '        if (statusType === "idle" || statusType === "error") setChildActive(parentSessionID, sessionID, false);',
    '      }',
    '      if (event.type === "session.idle" || event.type === "session.error") {',
    '        setChildActive(parentSessionID, sessionID, false);',
    '      }',
    '      return;',
    '    }',
    '',
    '    if (event.type === "message.updated") {',
    '      // Why: role is already cached above the parent lookup await so the',
    ...
    '',
    '    if (event.type === "session.idle" || event.type === "session.error") {',
    '      if (hasActiveChildren(sessionID)) return;',
