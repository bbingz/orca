...
// snappy. Prefer raster formats before SVG; SVG is last so a same-name PNG wins.
...
const REPO_ICON_FILE_EXTENSIONS = ['.png', '.webp', '.svg'] as const
...
function isSvgBuffer(buffer: Buffer): boolean {
  // Why: SVG is text; reject binary noise and require a root marker so random
  // files are not base64-encoded as icons. Rendered only via <img data:…>, not
  // inline HTML, so script in SVG does not execute in Chromium.
